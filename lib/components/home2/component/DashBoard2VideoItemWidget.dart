...
import 'package:gnews/components/VideoPlayDialog.dart';
  ...
  Widget build(BuildContext context) {
    Widget imageWidget() {
      if (videoData.image_url.validate().isNotEmpty) {
        return cachedImage(videoData.image_url.validate(), fit: BoxFit.cover)
            .cornerRadiusWithClipRRect(defaultRadius);
      } else if (videoData.video_type.validate() == VideoTypeYouTube) {
        return cachedImage(videoData.video_url.validate().getYouTubeThumbnail(),
                fit: BoxFit.cover)
            .cornerRadiusWithClipRRect(defaultRadius);
      }
      return Container(
          decoration: BoxDecoration(
              borderRadius: radius(defaultRadius), color: Colors.grey));
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: gray.withOpacity(0.4), blurRadius: 0.6, spreadRadius: 1.0)
        ],
      ),
      child: Stack(
        fit: StackFit.expand,
        children: [
          ...
          Container(color: Colors.black38)
              .cornerRadiusWithClipRRect(defaultRadius),
          Positioned(
            child: Text(videoData.title.validate(),
